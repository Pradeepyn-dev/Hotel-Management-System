...
BEGIN
    ...
    FROM payment
    WHERE id = NEW.id; -- Assuming there's a relationship between roombook and payment tables
...
-- Procedure 1: Get details of a booking based on the booking ID
DELIMITER //
...
CREATE TRIGGER before_insert_roombook
BEFORE INSERT ON roombook
FOR EACH ROW
BEGIN
    DECLARE available_rooms INT;
    DECLARE booked_rooms INT;

    -- Get the total number of available rooms
    SELECT COUNT(*) INTO available_rooms FROM room;

    -- Get the total number of booked rooms
    SELECT COUNT(*) INTO booked_rooms FROM roombook;

    -- Check if there are enough available rooms
    IF booked_rooms >= available_rooms THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No more rooms available';
    END IF;
END;
